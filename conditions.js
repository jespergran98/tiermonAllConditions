...
/**
 * Hierarchical Bayesian ranking using Beta-Binomial conjugate prior.
 * 
 * Algorithm overview:
 * 1. Derive global meta statistics (mean win rate, between-deck variance)
 * 2. Estimate Beta distribution prior parameters from these statistics
 * 3. Update each deck's Beta distribution with observed game data (posterior)
 * 4. Calculate conservative lower confidence bound using constant z-score
 * 
 * This produces ratings that:
 * - Regress extreme win rates toward the meta mean when sample size is small
 * - Reward consistent performance with large sample sizes
...
const hierarchicalBayesian = (allData) => {
  ...
  // Calculate basic statistics (treat ties as 50-50 splits for initial stats)
  ...
  // STEP 2: Estimate Beta distribution parameters via method of moments
  ...
  return allData.map(([origRank, deckName, wins, losses, ties]) => {
    ...
    // Maintain Beta-Binomial conjugacy by using integer counts first
    const postAlpha = priorAlpha + wins + 0.5 * ties;
    const postBeta = priorBeta + losses + 0.5 * ties;
    
    const posteriorMean = postAlpha / (postAlpha + postBeta);
    const posteriorVariance = (postAlpha * postBeta) / 
      ((postAlpha + postBeta) ** 2 * (postAlpha + postBeta + 1));
    
    // Calculate lower confidence bound (rating basis)
...
/**
 * Converts win percentage to 0-180 strength scale.
 * 50% win rate → 90 points, 60% → 108 points, etc.
 */
...
enrichedDecks.forEach((deck, index) => {
  ...
  // Rating based on lower confidence bound
  // Naturally incorporates Bayesian shrinkage and sample-size penalties
