...
const CONFIG = {
  bayesian: {
    ...
    // • Higher values (4+) = More conservative, harder for decks to rank high based on win rate
    // • Lower values (1.0-1.5) = Ratings are heavily impacted by win rates
    // • 2.0 = 95% confidence interval (standard choice)
    // Effect: Adjusts the overall value of win rate, higher value means win rates mean less
    winRateZScore: 4.5,
    ...
    // higher win rate weight makes ratings scores closer, but other numbers need adjusting
    winRateWeight: 0.95,
    shareWeight: 0.05,  // Must sum to 1.0 with winRateWeight
    
    // META ADJUSTMENT K (Dataset Size Sensitivity)
    // Controls how z-scores scale with total dataset size
    // • Higher values (3000+) = Takes longer to reach full confidence, more conservative overall
    ...
    metaAdjustmentK: 4000,
    
    // SHARE PENALTY K (Low Popularity Dampening)
    // Exponentially reduces ratings for decks with very low play rates
    // • Higher values (6.0+) = Less penalty, even rare decks can rank high if they perform well
    // • Lower values (2-3) = Severe penalty, only popular decks can rank high
    // Formula: penalty = 1 - e^(-k * share)
    // increase number if you want less played decks with high win rates to appear higher
    sharePenaltyK: 7,
...
const calculatePercentile = (value, sortedArray) => {
  ...
  return round3((rank / sortedArray.length) * 100);
};
...
const hierarchicalBayesianHybrid = (allData) => {
  ...
  return allData.map(deck => {
    ...
    // Return deck with rating only, no bayesianDetails
    return {
      ...deck,
      rating: round3(lowerBound * CONFIG.bayesian.ratingScale)
    };
  });
};
...
const calculateDeckStatistics = (rawDecks) => {
  ...
  if (!Array.isArray(rawDecks) || rawDecks.length === 0) {
    console.warn('No deck data provided to calculator');
    return [];
  }
  ...
  const withBasics = rawDecks.map(deck => {
    ...
    const adjusted_win_rate_raw = (deck.wins + CONFIG.bayesian.tieWeight * deck.ties) / total_matches;
    
    return {
      ...deck,
      total_matches,
      win_rate: round3((deck.wins / total_matches) * 100),
      adjusted_win_rate: round3(adjusted_win_rate_raw * 100),
      adjusted_win_rate_raw, // Keep for internal calculations
      avg_tournament_depth: round3((deck.wins + 0.5 * deck.ties) / deck.count)
    };
  });
  ...
  const withShares = withBasics.map(deck => ({
    ...deck,
    share: round3((deck.total_matches / totalGames) * 100),
    share_raw: deck.total_matches / totalGames, // Keep for internal calculations
    tournament_share: round3((deck.count / totalCount) * 100),
    share_compared_to_most_played_deck: round3(((deck.total_matches / totalGames) * 100) / mostPlayedShare * 100)
  }));
  ...
  const withBayesian = hierarchicalBayesianHybrid(withShares.map(deck => ({
    ...deck,
    meta_impact: round3(deck.adjusted_win_rate * deck.share)
  })));
  ...
  // Step 7: Add display formatting (0 decimals for rounded values)
...
enrichedDecks.slice(0, 10).forEach(deck => {
  ...
  console.log(`  Count (tournament entries): ${deck.count} (rounded: ${deck.count_rounded})`);
  ...
  console.log(`  Adjusted Win Rate (accounting for ties): ${deck.adjusted_win_rate.toFixed(2)}%`);
...
console.log(`Note: Showing top ${Math.min(10, enrichedDecks.length)} decks out of ${enrichedDecks.length} total decks analyzed.`);
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { calculateDeckStatistics, CONFIG, enrichedDecks };
} else if (typeof window !== 'undefined') {
  window.calculateDeckStatistics = calculateDeckStatistics;
